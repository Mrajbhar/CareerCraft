...
const FOOTER = `<div style="text-align:center;font:11px -apple-system,Segoe UI,sans-serif;color:#9aa3af;margin-top:14px">Made with CareerCraft — careercraft.app</div>`;
const FONTS =
...
export function exportPDF(opts = {}) {
  ...
  win.document.write(
    ...
    `<style>` +
      ...
      `@page{ size:auto; margin:0; }` +              /* margin:0 hides Chrome's date/title/URL header & footer */
      `body{ padding:10mm; }` +                       /* breathing room without giving the browser a margin to print into */
  ...
  // give the fonts/layout a moment to settle, then print
  setTimeout(() => { win.print(); }, 500);
}

// Word (.doc) export — builds a Word-openable HTML file from the rendered
// resume. Uses outerHTML so the page padding/margins come through.
export function exportWord(name, opts = {}) {
  const node = document.getElementById("resume-paper");
  if (!node) return;
  const html =
