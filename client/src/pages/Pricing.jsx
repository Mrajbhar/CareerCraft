...
export default function Pricing() {
  ...
  useEffect(() => {
    api
      ...
      .catch(() => setCatalog({ plans: [], configured: false }));
  }, []);
    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10 sm:py-14">
      <div className="text-center mb-10">
        ...
        <h1 className="font-display text-3xl sm:text-4xl font-semibold mt-4">
          Land the job faster with Pro
        </h1>
        <p className="text-ink2 mt-2 max-w-lg mx-auto">
          Start free. Upgrade when you want unlimited AI, clean exports, and the
          full interview toolkit.
        </p>
        {isPro && (
          <div className="inline-flex items-center gap-2 mt-5 px-4 py-2 rounded-full bg-brand text-white text-sm font-semibold">
            <Crown size={16} /> You're on Pro
            {user?.planExpires
              ? ` · renews/expires ${new Date(user.planExpires).toLocaleDateString()}`
              : ""}
          </div>
        )}
        {done && !isPro && (
          <p className="text-brand font-semibold mt-4">
            Payment received — enjoy Pro! 🎉
          </p>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 items-start">
        ...
        <div className="rounded-2xl border border-line bg-card p-6 sm:p-7">
          ...
          <div className="mt-2 mb-5">
            ...
            <span className="text-ink2"> / forever</span>
          </div>
          <ul className="flex flex-col gap-2.5 mb-6">
            {FREE_FEATURES.map((f) => (
              <li key={f} className="flex items-start gap-2.5 text-sm">
                <Check size={17} className="text-ink2 shrink-0 mt-0.5" />
        ...
        <div className="relative rounded-2xl border-2 border-brand bg-card p-6 sm:p-7 shadow-xl shadow-brand/10">
          <span className="absolute -top-3 left-6 inline-flex items-center gap-1.5 text-xs font-bold bg-brand text-white px-3 py-1 rounded-full">
          ...
          {yearly && (
            <p className="text-xs text-ink2 mb-5">
              or ₹{yearly.price}/year — save{" "}
              {Math.round(
                (1 - yearly.price / ((monthly?.price ?? 199) * 12)) * 100,
              )}
              %
            </p>
          )}
      ...
      {sprint && (
        <div className="mt-5 rounded-2xl border border-line bg-card p-5 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="grid place-items-center w-11 h-11 rounded-xl bg-rust/10">
              <Zap className="text-rust" size={22} />
            </div>
            <div>
              ...
              <p className="text-sm text-ink2">
                Job hunting right now? Get full Pro access for {sprint.days}{" "}
                days — no subscription.
              </p>
            </div>
          </div>
          <button
            onClick={() => upgrade("sprint")}
            disabled={isPro || busy}
          ...
          >
            ...
            ₹{sprint.price} for {sprint.days} days
          </button>
        </div>
      )}

      {catalog && catalog.configured === false && (
        <p className="text-center text-xs text-ink2 mt-6">
          Note: payments aren't configured yet — add your Razorpay keys to{" "}
          <span className="font-mono">server/.env</span> to enable checkout.
        </p>
      )}
      <p className="text-center text-xs text-ink2 mt-6">
        Secure payments via Razorpay. Cancel anytime — Pro stays active until it
        expires.
      </p>
    </div>
  );
