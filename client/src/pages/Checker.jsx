...
function jdMatch(resumeText, jd) {
  ...
  const freq = {};
  toks(jd).forEach((w) => { if (w.length > 2 && !JD_STOP.has(w)) freq[w] = (freq[w] || 0) + 1; });
  const keywords = Object.keys(freq).sort((a, b) => freq[b] - freq[a]).slice(0, 28);
...
export default function Checker() {
  ...
  const analyze = async (f) => {
    setUpErr(""); setResult(null); setResumeText(""); setJdResult(null); setFile(f); setAnalyzing(true);
    try {
          ...
          {resumeText && !analyzing && (
            <div className="mt-5 rounded-2xl border border-line bg-card p-5 sm:p-6">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <h3 className="font-display text-lg font-semibold">Match against a job description</h3>
              </div>
              <p className="text-sm text-ink2 mb-3">Paste the job post below — we’ll show how many of its keywords your resume already covers, and which ones are missing.</p>
              <textarea
                value={jd}
              ...
              {jdResult && (
                <div className="mt-5">
                  <div className="flex items-center gap-4 pb-4 border-b border-line">
                    <div className="font-display text-3xl font-semibold" style={{ color: jdResult.pct >= 70 ? "#15634f" : jdResult.pct >= 45 ? "#d4a23a" : "#b04a36" }}>
                      {jdResult.pct}%
                    </div>
                    <div className="text-sm text-ink2">
                      keyword match · {jdResult.matched.length} of {jdResult.matched.length + jdResult.missing.length} key terms found in your resume
                    </div>
                  </div>
                  {jdResult.missing.length > 0 && (
                    <div className="mt-4">
                      <div className="text-xs uppercase tracking-wide text-ink2 font-semibold mb-2">Missing keywords — add these if they’re true for you</div>
                      <div className="flex flex-wrap gap-1.5">
                        {jdResult.missing.map((k) => (
                          <span key={k} className="px-2.5 py-1 rounded-full text-xs font-medium border border-rust/30 bg-rust/5 text-rust">{k}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {jdResult.matched.length > 0 && (
                    <div className="mt-4">
                      <div className="text-xs uppercase tracking-wide text-ink2 font-semibold mb-2">Already covered</div>
                      <div className="flex flex-wrap gap-1.5">
                        {jdResult.matched.map((k) => (
                          <span key={k} className="px-2.5 py-1 rounded-full text-xs font-medium border border-brand/30 bg-brand/5 text-brand">{k}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  <p className="text-xs text-ink2 mt-4 pt-3 border-t border-line">Tip: only add keywords that genuinely match your skills — never keyword-stuff. This is a guide, not a guarantee.</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="rounded-2xl border border-line bg-card p-5 sm:p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1">
                  <Select theme="dark" width="100%" value={selected} onChange={setSelected}
                    placeholder="Select a resume"
                    options={resumes.map((r) => ({ value: r._id, label: `${r.title} · ${r.template}` }))} />
                </div>
