...
export default function Builder() {
  ...
  const aiSummary = async () => {
    ...
    try {
      ...
      handleAiErr(err, "AI request failed.");
    }
    flag("summary", false);
  };
  ...
  const aiReview = async () => {
    ...
    try {
      ...
      setReview(null);
      handleAiErr(err, "AI review failed.");
    }
    setReviewing(false);
  };
  ...
  const genTailor = async () => {
    ...
    try {
      ...
      handleAiErr(err, "Tailoring failed. Please try again.");
    }
    ...
    <div className="min-h-[calc(100vh-64px)]">
      ...
      {(reviewing || review !== null) && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-paper border border-line rounded-2xl w-full max-w-xl max-h-[80vh] overflow-auto p-6 shadow-2xl"
          >
      ...
      {tailorOpen && (
        <div
          onClick={() => !tailorLoading && setTailorOpen(false)}
          className="fixed inset-0 z-50 grid place-items-center p-5"
          style={{ background: "rgba(0,0,0,.45)" }}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-paper border border-line rounded-2xl w-full max-w-xl max-h-[88vh] overflow-auto p-6 shadow-2xl"
          >
            ...
            <p className="text-sm text-ink2 mb-4">
              Paste a job description — AI rewrites your summary, suggests
              matching bullet points, and lists keywords to include.
            </p>
            <Field
              area
              label="Job description"
              value={tailorJD}
              onChange={setTailorJD}
            ...
            >
              {tailorLoading ? (
                ...
                <>
                  <Sparkles size={15} />{" "}
                  {tailorResult ? "Regenerate" : "Tailor my resume"}
                </>
              )}
            </button>
              ...
              <div className="mt-5 flex flex-col gap-5">
                {tailorResult.summary && (
                  <div>
                    ...
                    <div className="rounded-xl border border-line bg-paper/40 p-3.5 text-sm leading-relaxed text-ink">
                      {tailorResult.summary}
                    </div>
                  </div>
                )}
                {Array.isArray(tailorResult.bullets) &&
                ...
                {Array.isArray(tailorResult.keywords) &&
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {tailorResult.keywords.map((k, i) => (
                          <span
                            key={i}
                            className="text-xs font-semibold px-2 py-1 rounded-full bg-brand/10 text-brand"
                          >
                            {k}
                          </span>
                        ))}
      ...
      {coverOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-paper border border-line rounded-2xl w-full max-w-xl max-h-[85vh] overflow-auto p-6 shadow-2xl"
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-display text-xl font-semibold flex items-center gap-2">
