...
import {
  ...
  X, ChevronDown, Eye, PencilLine, ArrowLeft,
} from "lucide-react";
import api from "../api";
import { Field, AiBtn, AddBtn, Card, DownloadBtn, BulletEditor } from "../components/ui";
...
export default function Builder() {
  ...
  const addCustomItem = (sid) => setData((d) => ({ ...d, custom: d.custom.map((s) => s.id === sid ? { ...s, items: [...s.items, { id: Date.now(), heading: "", subheading: "", period: "", bullets: "" }] } : s) }));
  ...
  const previewData = { ...data, name: data.name || `${data.firstName || ""} ${data.lastName || ""}`.trim() };
    ...
    <div className="min-h-[calc(100vh-64px)]">
      ...
      <div className="lg:flex lg:items-start">
        ...
        <div className={`builder-editor ${tab === "edit" ? "flex" : "hidden"} lg:flex flex-col gap-3 w-full lg:w-[500px] xl:w-[540px] lg:shrink-0 lg:h-[calc(100vh-140px)] lg:overflow-y-auto lg:border-r lg:border-line px-4 sm:px-6 py-4`}>
          ...
          <Acc {...acc("experience")} icon={Briefcase} title="Work Experience" count={data.experience.length} action={<AddBtn onClick={() => add("experience", { role: "", company: "", location: "", period: "", bullets: "" })} />}>
            ...
            {data.experience.map((e) => (
              <Card key={e.id} onDel={() => del("experience", e.id)}>
                <div className="grid grid-cols-2 gap-2.5">
                  ...
                  <Field label="Period" value={e.period} onChange={(v) => upd("experience", e.id, "period", v)} placeholder="2022 — Present" />
                </div>
          ...
          <Acc {...acc("education")} icon={GraduationCap} title="Education" count={data.education.length} action={<AddBtn onClick={() => add("education", { school: "", degree: "", period: "", detail: "" })} />}>
            {data.education.map((e) => (
              <Card key={e.id} onDel={() => del("education", e.id)}>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  ...
                  <Field label="Period" value={e.period} onChange={(v) => upd("education", e.id, "period", v)} />
                </div>
            ...
            <Acc key={sec.id} {...acc("custom-" + sec.id)} icon={LayoutList} title={sec.title || "Section"} count={(sec.items || []).length}
              ...
              {(sec.items || []).map((it) => (
                <Card key={it.id} onDel={() => delCustomItem(sec.id, it.id)}>
                  ...
                  <Field label="Period" value={it.period} onChange={(v) => updCustomItem(sec.id, it.id, "period", v)} />
        ...
        <div className={`${tab === "preview" ? "block" : "hidden"} lg:block lg:flex-1 lg:h-[calc(100vh-140px)] lg:overflow-y-auto px-4 sm:px-6 lg:px-8 py-4`}>
          <div className="flex justify-between items-center mb-3 flex-wrap gap-2 w-full max-w-[1100px] mx-auto">
            <div className="flex gap-1.5 flex-wrap">
              {["classic", "modern", "professional", "standard", "minimal", "elegant", "compact"].map((t) => (
