// ===> place at: client/src/pages/Builder.jsx
...
import {
  ...
  X, ChevronDown, Eye, PencilLine, ArrowLeft, Mail, Copy, Target, Crown, Lock, History, Save, RotateCcw,
} from "lucide-react";
import api from "../api";
...
const ACCENTS = ["#15634f", "#1d4ed8", "#4f46e5", "#7c3aed", "#b91c1c", "#9d174d", "#0f766e", "#334155"];
...
function Acc({ id, icon: Icon, title, count, action, open, setOpen, children, filled }) {
  const isOpen = open === id;
  const hasStatus = filled !== undefined || count !== undefined;
  const done = filled !== undefined ? filled : (count || 0) > 0;
  const statusLabel = filled !== undefined ? (done ? "Complete" : "Empty") : (count > 0 ? `${count} added` : "Empty");
  return (
    <div className="rounded-xl border border-line bg-card overflow-hidden shrink-0">
      <div className="flex items-center justify-between gap-2 pr-3">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          {hasStatus && !isOpen && (
            <span className={`hidden sm:inline-flex items-center gap-1.5 text-[11px] font-medium ${done ? "text-brand" : "text-ink2"}`}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: done ? "#18a884" : "rgba(255,255,255,.22)" }} />
              {statusLabel}
            </span>
          )}
          {isOpen && action}
...
export default function Builder() {
  ...
  const openHistory = async () => { setHistOpen(true); try { const { data } = await api.get(`/resumes/${id}/versions`); setVersions(data); } catch {} };
  const saveVersion = async () => { setHistBusy(true); try { const { data } = await api.post(`/resumes/${id}/versions`, { label: "Manual save" }); setVersions(data); } catch {} setHistBusy(false); };
  const restoreVer = async (vid) => { setRestoring(vid); try { const { data: r } = await api.post(`/resumes/${id}/versions/${vid}/restore`); setDocTitle(r.title || "Untitled Resume"); setTemplate(r.template || "classic"); setData(migrate(r.data)); setHistOpen(false); } catch {} setRestoring(null); };
  ...
  const openTailor = () => { if (!isPro) { setUpgradeFor("Tailor to Job"); return; } setTailorOpen(true); };
  ...
  const _cChecks = [
    !!((data.firstName || data.name || "").trim()), !!(data.summary || "").trim(),
    (data.experience || []).length, (data.skillGroups || []).length,
    (data.education || []).length,
    ((data.projects || []).length + (data.certifications || []).length + (data.achievements || []).length),
  ];
  const completeness = Math.round((_cChecks.filter(Boolean).length / _cChecks.length) * 100);
  const fontLocked = template === "typeset" || template === "executive";
    ...
    <div className="min-h-[calc(100vh-64px)]">
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-4">
        <div className="flex items-center gap-2 sm:gap-3 rounded-2xl border border-line bg-card px-3 sm:px-4 py-2.5">
          ...
          <button onClick={openHistory} title="Version history" aria-label="Version history"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-line bg-paper text-ink text-sm font-semibold hover:bg-card transition shrink-0">
      ...
      <div className="px-4 sm:px-6 lg:px-8 mt-3">
        <div className="relative z-30 flex items-center gap-3 flex-wrap justify-between rounded-2xl border border-line bg-card px-3 sm:px-4 py-2.5">
          <div className="flex gap-1.5 flex-wrap">
            {["classic", "modern", "professional", "standard", "minimal", "elegant", "compact", "executive", "typeset", "aurora", "onyx"].map((t) => {
              const isProTpl = PRO_TEMPLATES.includes(t);
              const locked = isProTpl && !isPro;
              return (
                <button key={t} onClick={() => (locked ? proGate("This template") : setTemplate(t))} title={locked ? "Pro template" : ""}
                  ...
                  {t}{isProTpl && <Crown size={11} className={template === t ? "text-amber-300" : "text-brand"} />}
                </button>
              );
            })}
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            {fontLocked ? (
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-ink2 hidden xl:inline">Font</span>
                <span className="text-xs text-ink2 border border-line rounded-lg px-2.5 py-1.5 bg-paper/50">Computer Modern <span className="opacity-60">· fixed</span></span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-ink2 hidden xl:inline">Font</span>
                <Select theme="dark" width={170} value={data.font || ""} onChange={(v) => set("font", v)}
                  options={FONT_CHOICES.map((f) => ({ value: f.id, label: f.label }))} />
              </div>
            )}
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-ink2 hidden xl:inline">Size</span>
              <div className="flex items-center rounded-lg border border-line bg-paper overflow-hidden">
                <button onClick={() => set("fontScale", Math.max(0.8, Math.round(((data.fontScale || 1) - 0.05) * 100) / 100))} className="px-2.5 py-1 text-ink2 hover:bg-card text-sm font-bold" title="Smaller">−</button>
                <span className="px-2 text-xs font-semibold text-ink text-center" style={{ minWidth: 42 }}>{Math.round((data.fontScale || 1) * 100)}%</span>
                <button onClick={() => set("fontScale", Math.min(1.3, Math.round(((data.fontScale || 1) + 0.05) * 100) / 100))} className="px-2.5 py-1 text-ink2 hover:bg-card text-sm font-bold" title="Bigger">+</button>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-ink2 items-center gap-1 hidden xl:inline-flex">Accent{!isPro && <Crown size={11} className="text-brand" />}</span>
              <div className="flex items-center gap-1.5">
                {ACCENTS.map((c) => (
                  <button key={c} aria-label="accent color" title={isPro ? "" : "Pro feature"}
                    onClick={() => (isPro ? set("accent", c) : proGate("Custom accent colors"))}
                    className="w-5 h-5 rounded-full border-2 transition"
                    style={{ background: c, borderColor: (data.accent || "#15634f") === c ? "#1b1a17" : "transparent", opacity: isPro ? 1 : 0.55 }} />
                ))}
              </div>
            </div>
      ...
      {histOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4" style={{ background: "rgba(0,0,0,.6)" }} onClick={() => setHistOpen(false)}>
          <div className="w-full max-w-md rounded-2xl border border-line bg-card p-5 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              <h3 className="font-display text-lg font-semibold flex items-center gap-2"><History size={18} /> Version history</h3>
              <button onClick={() => setHistOpen(false)} aria-label="Close" className="text-ink2 hover:text-ink p-1"><X size={18} /></button>
            </div>
            <p className="text-ink2 text-[13px] mb-3">Snapshots of this resume. Restoring saves your current version first, so you can always undo.</p>
            <button onClick={saveVersion} disabled={histBusy}
              ...
              {histBusy ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Save current as version
            </button>
            <div className="flex-1 overflow-y-auto -mx-1 px-1">
              {versions.length === 0 ? (
                <p className="text-ink2 text-sm text-center py-6">No versions yet.</p>
              ) : (
                <ul className="flex flex-col divide-y divide-line">
                  {versions.map((v) => (
                    <li key={v._id} className="flex items-center gap-3 py-2.5">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium truncate">{v.label || "Saved version"}</div>
                        <div className="text-xs text-ink2">{new Date(v.createdAt).toLocaleString()} · {v.template}</div>
                      </div>
                      <button onClick={() => restoreVer(v._id)} disabled={restoring === v._id}
                        ...
                        {restoring === v._id ? <Loader2 size={13} className="animate-spin" /> : <RotateCcw size={13} />} Restore
                      </button>
                    </li>
                  ))}
                </ul>
      ...
      <div className="lg:flex lg:items-start">
        ...
        <div className={`builder-editor ${tab === "edit" ? "flex" : "hidden"} lg:flex flex-col gap-3 w-full lg:w-[500px] xl:w-[540px] lg:shrink-0 lg:h-[calc(100vh-140px)] lg:overflow-y-auto lg:border-r lg:border-line px-4 sm:px-6 py-4`}>
          ...
          <Acc {...acc("personal")} icon={User} title="Personal Information" filled={!!((data.firstName || "").trim() || (data.name || "").trim())}>
              ...
              {PREFS.map(([k, l]) => {
                ...
                return (
                  <button key={k} onClick={() => pref(k)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition ${on ? "bg-brand text-white border-brand" : "bg-paper text-ink2 border-line hover:border-brand"}`}>{l}</button>
                );
              })}
            </div>
          </Acc>
          ...
          <Acc {...acc("summary")} icon={Sparkles} title="Professional Summary" filled={!!(data.summary || "").trim()} action={<AiBtn onClick={aiSummary} loading={busy.summary} label="AI write" />}>
          ...
          {data.custom.map((sec) => (
            <Acc key={sec.id} {...acc("custom-" + sec.id)} icon={LayoutList} title={sec.title || "Section"} count={(sec.items || []).length}
              ...
              <button onClick={() => del("custom", sec.id)} className="text-xs font-semibold text-rust border border-line bg-paper rounded-lg px-3 py-1.5 mt-1 self-start">Remove section</button>
            </Acc>
          ))}
        ...
        <div className={`${tab === "preview" ? "block" : "hidden"} lg:block lg:flex-1 lg:h-[calc(100vh-140px)] lg:overflow-y-auto px-4 sm:px-6 lg:px-8 py-4`}>
          <div className="flex justify-between items-center mb-3 w-full max-w-[1100px] mx-auto">
            <span className="text-xs font-semibold text-ink2 inline-flex items-center gap-1.5"><Eye size={14} /> Live preview</span>
            <div className="hidden sm:flex items-center gap-2" title="Resume completeness">
              <div style={{ position: "relative", width: 36, height: 36 }}>
                <svg width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="15" fill="none" stroke="rgba(255,255,255,.1)" strokeWidth="4" /><circle cx="18" cy="18" r="15" fill="none" stroke="#18a884" strokeWidth="4" strokeLinecap="round" strokeDasharray="94.2" strokeDashoffset={94.2 - (94.2 * completeness) / 100} transform="rotate(-90 18 18)" /></svg>
                <span style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontSize: 9, fontWeight: 700 }}>{completeness}%</span>
              </div>
              <span className="text-xs text-ink2">complete</span>
            </div>
          </div>
          <div className="w-full max-w-[1100px] mx-auto rounded-xl px-3 sm:px-8 pt-4 pb-10"
            style={{ backgroundImage: "radial-gradient(rgba(255,255,255,.05) 1px, transparent 1px)", backgroundSize: "22px 22px" }}>
            {/* realistic paper sheet on a desk */}
            <div className="relative mx-auto" style={{ maxWidth: 850 }}>
              {/* faint stacked pages behind, like a real multi-page document */}
              <div aria-hidden className="absolute inset-0 rounded-[3px] bg-white/25" style={{ transform: "translate(9px, 12px)", boxShadow: "0 30px 60px -28px rgba(0,0,0,.6)" }} />
              <div aria-hidden className="absolute inset-0 rounded-[3px] bg-white/60" style={{ transform: "translate(4px, 6px)" }} />
              {/* the page */}
              <div className="relative rounded-[3px] overflow-hidden bg-white transition-shadow duration-300"
                style={{ boxShadow: "0 2px 4px rgba(0,0,0,.25), 0 12px 24px -8px rgba(0,0,0,.45), 0 45px 90px -30px rgba(0,0,0,.85), 0 0 0 1px rgba(0,0,0,.06)" }}>
                <ResumePreview r={previewData} template={template} />
                {/* soft sheen from top light */}
                <div className="pointer-events-none absolute inset-x-0 top-0 h-28" style={{ background: "linear-gradient(180deg, rgba(255,255,255,.55), transparent)", mixBlendMode: "soft-light" }} />
                {/* very faint paper grain */}
                <div className="pointer-events-none absolute inset-0 opacity-[0.04]" style={{ backgroundImage: "radial-gradient(rgba(0,0,0,.6) 0.5px, transparent 0.5px)", backgroundSize: "3px 3px" }} />
                {/* subtle left-edge gutter shadow, like a bound page */}
                <div className="pointer-events-none absolute inset-y-0 left-0 w-6" style={{ background: "linear-gradient(90deg, rgba(0,0,0,.05), transparent)" }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <UpgradeModal open={!!upgradeFor} feature={upgradeFor} onClose={() => setUpgradeFor(null)} onUpgrade={() => { setUpgradeFor(null); nav("/pricing"); }} />
