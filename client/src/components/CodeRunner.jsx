...
function runJS(code, stdin) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      resolve({
        stdout: "",
        stderr: "Timed out after 8s (possible infinite loop).",
      });
    }, 8000);
    worker.onmessage = (e) => {
      clearTimeout(timer);
    ...
    worker.onerror = (ev) => {
      clearTimeout(timer);
      worker.terminate();
      resolve({ stdout: "", stderr: ev.message || "Error" });
    };
...
function getPyodide() {
  if (!window.__pyodidePromise) {
    window.__pyodidePromise = (async () => {
      await loadScript(
        `https://cdn.jsdelivr.net/pyodide/${PYODIDE_VER}/full/pyodide.js`,
      );
...
function wandboxList() {
  if (!wandboxListPromise)
    wandboxListPromise = fetch("https://wandbox.org/api/list.json").then((r) =>
      r.json(),
    );
...
async function resolveCompiler(langKey) {
  ...
  if (langKey === "c++")
    return (
      find((c) => c.language === "C++" && /^gcc/.test(c.name)) ||
      find((c) => c.language === "C++")
    );
  if (langKey === "c")
    return (
...
async function runWandbox(langKey, code, stdin) {
  ...
  if (!compiler)
    throw new Error(
      "That compiler isn't available right now — try again later.",
    );
...
async function gbCompiler(lang) {
  ...
  const res = await fetch(
    `${GODBOLT}/compilers/${lang}?fields=id,name,supportsExecute`,
...
async function runGodbolt(lang, code, stdin) {
  ...
  if (!res.ok)
    throw new Error("Compiler service request failed (" + res.status + ").");
  const d = await res.json();
  const stdout = (d.stdout || []).map((l) => l.text).join("\n");
  const compileErr = ((d.buildResult && d.buildResult.stderr) || [])
    .map((l) => l.text)
    .join("\n");
...
function parseVerdict(stdout) {
  ...
  for (const ln of (stdout || "").split("\n")) {
    if (ln.startsWith("__T")) {
      const m = ln.match(
        /^__T(\d+):(PASS|FAIL)\|got=([\s\S]*?)\|exp=([\s\S]*)$/,
      );
      if (m)
        cases.push({ i: +m[1], ok: m[2] === "PASS", got: m[3], exp: m[4] });
...
export default function CodeRunner({
  ...
  const startKey =
    initialLangKey && LANGS.some((l) => l.key === initialLangKey)
      ? initialLangKey
      : "javascript";
  const [langKey, setLangKey] = useState(startKey);
  const [code, setCode] = useState(
    initialCode ??
      (LANGS.find((l) => l.key === startKey)?.sample || LANGS[0].sample),
  );
  ...
  const pickLang = (k) => {
    setLangKey(k);
    setCode(LANGS.find((l) => l.key === k).sample);
    setOut(null);
    setErr("");
    setVerdict(null);
  };
  ...
  const run = async () => {
    ...
    try {
      ...
      if (lang.engine === "js") r = await runJS(code, stdin);
        ...
        r = await runPython(code, stdin);
        setStatus("");
      } else if (lang.engine === "wandbox") {
        setStatus("Compiling & running…");
        r = await runWandbox(langKey, code, stdin);
        setStatus("");
      } else {
        setStatus("Compiling & running…");
        r = await runGodbolt(lang.key, code, stdin);
        setStatus("");
      }
  ...
  const runTests = async () => {
    ...
    try {
      ...
      setVerdict(v);
      if (v.total > 0 && onResult)
        onResult(v.accepted ? "Accepted" : "Wrong Answer");
  ...
  const hl = (c) =>
    highlight(c, languages[lang.prism] || languages.clike, lang.prism);
    ...
    <div className="rounded-2xl border border-line overflow-hidden bg-card">
      <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 border-b border-line flex-wrap">
        <div className="relative">
          ...
          <ChevronDown
            size={15}
            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-ink2 pointer-events-none transition-transform"
            style={{
              transform: langOpen
          ...
          {langOpen && (
            <>
              ...
              />
              <div
                className="absolute z-20 mt-1.5 w-44 rounded-xl border border-white/12 overflow-hidden shadow-2xl py-1"
              ...
              >
                {LANGS.map((l) => (
                  <button
                    ...
                    style={{
                      color: l.key === langKey ? "#18a884" : "#cfd4de",
                      background:
                        l.key === langKey
                          ? "rgba(24,168,132,.12)"
                          : "transparent",
        ...
        <button
          ...
          className={`px-3 py-2 rounded-lg text-sm font-semibold border transition ${showStdin ? "bg-ink text-white border-ink" : "bg-white text-ink2 border-line hover:bg-paper"}`}
        >
          Input
        </button>
        ...
        <button
          onClick={reset}
          title="Reset"
          className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-line bg-white text-ink2 text-sm font-semibold hover:bg-paper transition"
        >
          <RotateCcw size={15} />{" "}
        ...
        {canJudge && langKey === "python" && (
          <button
            onClick={runTests}
            disabled={running}
            title="Run your solution against the test cases"
      ...
      {verdict && (
        <div
          className="px-3 sm:px-4 py-3 border-b border-line"
          style={{
            background: verdict.accepted
              ? "rgba(24,168,132,.08)"
              : "rgba(224,116,58,.08)",
          }}
        >
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            {verdict.accepted ? (
              ...
              <X size={18} style={{ color: "#e0743a" }} />
            )}
            <span
              className="font-bold"
              style={{ color: verdict.accepted ? "#18a884" : "#e0743a" }}
            >
              {verdict.accepted
                ? "Accepted"
                : verdict.total
          ...
          <div className="flex flex-wrap gap-1.5">
            {verdict.cases.map((c) => (
              <span
                key={c.i}
                title={c.ok ? "Passed" : `got ${c.got} · expected ${c.exp}`}
                className="text-[11px] font-bold px-2 py-0.5 rounded-full"
                style={{
                  color: c.ok ? "#18a884" : "#e0743a",
                  background: (c.ok ? "#18a884" : "#e0743a") + "1a",
                }}
              >
                Case {c.i + 1} {c.ok ? "\u2713" : "\u2717"}
              </span>
          ...
          {!verdict.accepted && verdict.cases.find((c) => !c.ok) && (
            ...
            >
              First failing → got: {verdict.cases.find((c) => !c.ok).got} ·
              expected: {verdict.cases.find((c) => !c.ok).exp}
            </div>
          )}
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-2">
        <div
          ...
          style={{ background: "#1b1a17", maxHeight: 480 }}
        >
          <Editor
            value={code}
          ...
          />
          <span className="absolute top-2 right-3 text-[10px] uppercase tracking-wide text-white/40 pointer-events-none">
            {lang.label}
          </span>
        </div>
        <div
          className="border-t lg:border-t-0 lg:border-l border-line flex flex-col"
          style={{ background: "#14130f" }}
        >
          <div className="flex items-center gap-1.5 px-4 py-2 text-[11px] uppercase tracking-wide text-white/45 border-b border-white/10">
          ...
          <div className="p-4 font-mono text-[13px] leading-relaxed overflow-auto h-[260px] sm:h-[380px]">
            {running ? (
              ...
              <span className="text-white/35">
                Press <strong className="text-white/60">Run</strong> (or Ctrl/⌘
                + Enter) to execute your code.
              </span>
            )}
          </div>
        </div>
