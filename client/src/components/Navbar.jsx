...
function Avatar({ user, size = 34 }) {
  ...
  return user?.avatar ? (
    ...
    <span
      className="grid place-items-center rounded-full text-white font-semibold ring-2 ring-white/15"
      style={{
        width: size,
        height: size,
        fontSize: size * 0.42,
...
export default function Navbar() {
  ...
  return (
    <header
      className={`no-print sticky top-0 z-30 bg-card/80 backdrop-blur-xl transition-shadow duration-300 ${scrolled ? "shadow-[0_8px_30px_-12px_rgba(0,0,0,.5)]" : ""}`}
    >
      {/* gradient hairline */}
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent, rgba(24,168,132,.5), rgba(45,212,191,.4), transparent)",
        }}
      />
      ...
      <div className="relative w-full px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        >
          <span
            className="relative grid place-items-center w-9 h-9 rounded-xl text-white transition-transform duration-300 group-hover:scale-105 group-hover:-rotate-6 shadow-lg shadow-brand/30"
        ...
        {user ? (
          <>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              >
                <span
                  className="absolute top-1 bottom-1 rounded-lg transition-all duration-300 ease-out shadow-lg shadow-brand/30"
                  style={{
                    left: pill.left,
                    width: pill.width,
                    opacity: pill.show ? 1 : 0,
                    background: "linear-gradient(135deg,#18a884,#15917a)",
                  }}
                />
              ...
              <div className="relative">
                ...
                >
                  <Avatar user={user} />
                  <span className="text-sm font-semibold text-ink hidden lg:inline">
                    {firstName}
                  </span>
                  <ChevronDown
                    size={15}
                ...
                {profileOpen && (
                  <>
                    ...
                    />
                    <div
                      className="absolute right-0 mt-2 w-60 z-20 rounded-2xl border border-line bg-card shadow-2xl overflow-hidden"
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className="md:hidden grid place-items-center w-10 h-10 rounded-lg border border-line bg-white active:scale-95 transition"
              aria-label="Menu"
            >
