...
export const getPlans = (req, res) => {
  res.json({
    ...
    configured: hasKeys(),
    plans: Object.values(PLANS).map(({ key, label, price, days }) => ({
      key,
      label,
      price,
      days,
...
export const createOrder = async (req, res) => {
  if (!hasKeys())
    return res.status(400).json({
      msg: "Payments not configured",
      error:
        "Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to server/.env, then restart the server.",
    });
  ...
  try {
    ...
    return res.status(500).json({
      msg: "Razorpay not installed",
      error:
        "Run `npm install razorpay` in the server folder, then restart the server.",
    });
  }
  try {
    ...
    res.status(500).json({
      ...
      error:
        e?.error?.description ||
        e?.message ||
        "Check that your Razorpay key id/secret are correct.",
    });
  }
};
...
export const verifyPayment = async (req, res) => {
  if (!hasKeys())
    return res.status(400).json({ msg: "Payments not configured." });
