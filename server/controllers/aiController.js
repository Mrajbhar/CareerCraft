...
async function modelFor(userId) {
  try { const u = await User.findById(userId); return isPro(u) ? "gemini-2.5-pro" : MODEL; } catch { return MODEL; }
}
const hasKey = () => !!apiKey();

async function ask(prompt, maxTokens = 1000, model = MODEL) {
  const isFlash = model.includes("flash");
  // Gemini 2.5 "thinks" before answering, and thinking tokens count against the
  // output budget. Disable thinking on flash; cap it (with headroom) on pro —
  // otherwise answers get cut short.
  const genConfig = {
    temperature: 0.7,
...
// POST /api/ai/email — career email writer (Pro gets the better model)
export const writeEmail = async (req, res) => {
  if (!hasKey()) return res.status(400).json({ msg: "AI not configured", error: "Add GEMINI_API_KEY to server/.env and restart the server." });
  const { kind = "follow-up", details = "" } = req.body;
  if (!details.trim()) return res.status(400).json({ msg: "Add a few details about the situation." });
  try {
    ...
    res.json({ text });
  } catch (e) { res.status(500).json({ msg: "AI error", error: e.message }); }
};

// POST /api/ai/linkedin — rewrite a LinkedIn About section
export const optimizeLinkedIn = async (req, res) => {
  if (!hasKey()) return res.status(400).json({ msg: "AI not configured", error: "Add GEMINI_API_KEY to server/.env and restart the server." });
  const { about = "" } = req.body;
  if (about.trim().length < 20) return res.status(400).json({ msg: "Paste your current LinkedIn About (a couple of lines)." });
  try {
    const model = await modelFor(req.userId);
    const text = await ask(
...
// POST /api/ai/quantify — turn weak bullets into quantified, action-led ones
export const quantifyBullets = async (req, res) => {
  if (!hasKey()) return res.status(400).json({ msg: "AI not configured", error: "Add GEMINI_API_KEY to server/.env and restart the server." });
  const { bullets = "" } = req.body;
  if (!bullets.trim()) return res.status(400).json({ msg: "Paste one or more bullet points (one per line)." });
  try {
    const model = await modelFor(req.userId);
    const text = await ask(
...
// POST /api/ai/gap — gap analysis between a resume and a job description
export const gapAnalysis = async (req, res) => {
  if (!hasKey()) return res.status(400).json({ msg: "AI not configured", error: "Add GEMINI_API_KEY to server/.env and restart the server." });
  const { resume = "", jobDescription = "" } = req.body;
  if (resume.trim().length < 30 || jobDescription.trim().length < 30)
    return res.status(400).json({ msg: "Paste both your resume text and the job description." });
  try {
    const model = await modelFor(req.userId);
    const text = await ask(
...
// POST /api/ai/hint — a single nudge for a coding problem (free)
export const solveHint = async (req, res) => {
  if (!hasKey()) return res.status(400).json({ msg: "AI not configured", error: "Add GEMINI_API_KEY to server/.env and restart the server." });
  const { title = "", statement = "" } = req.body;
  if (!title && !statement) return res.status(400).json({ msg: "Missing problem." });
  try {
    const model = await modelFor(req.userId);
    const text = await ask(
...
// POST /api/ai/solution — full editorial for a coding problem (Pro)
